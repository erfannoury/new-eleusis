...
def getRulesForSequence(cards):
    ...
    :return: returns a list of all possible rules the cards have in common
        (as trees)
    """
...
def getRulesForThreeCards(cards):
    ...
    :param cards:
    :return: a list of three-card rules that describe all the cards
        (as strings)
    """
    prev_curr = getRulesForTwoCards([cards[1], cards[2]])
    prev2_prev = getRulesForTwoCards(
        ([cards[0], cards[1]]), type1="previous2", type2="previous")
    three_rules = []
    ...
    for rule1 in prev2_prev:
        ...
        for rule2 in prev_curr:
            ...
            elif start1 in ["less(value(", "greater(value(", "equal(value("] \
            and start2 in ["less(value(", "greater(value(", "equal(value("]:
                use = True
            # match up rules that are both comparing suites
            elif start1 in ["less(suit(", "greater(suit(", "equal(suit("] \
            and start2 in ["less(suit(", "greater(suit(", "equal(suit("]:
                use = True
            # match up rules that are counting the differences between values
            elif (start1.startswith("equal(minus1(") or
                  start1.startswith("equal(plus1(")) \
            and (start2.startswith("equal(minus1(") or
                 start2.startswith("equal(plus1(")):
                use = True
    ...
    for func in same_funcs:
        if same_funcs[func](cards[0]) == same_funcs[func](cards[1]):
            all_pair_rules.append(
                "equal("+func+"("+type1+"), "+func+"("+type2+"))")
        else:
            all_pair_rules.append(
                "not(equal(" + func + "(" + type1 + "), " + func + "(" +
                type2 + ")))")
...
def combineListOfRules(ruleList):
    """
    takes a 2d list of rules and makes an overall rule that ands any inner
    lists and ors any outer lists
...
def getRandomRule():
    ...
    :return:
    """
    theRule = getMiniRandomRule()
    # "iff("+self.getMiniRandomRule()+", True, False)"
    print(theRule)
...
def getMiniRandomRule(rand=0):
    ...
    :param rand: this tells the function which type of rule to make.
    This is only set when a larger rule wants to create specific smaller rules
    :return:
    """
    ...
    two_items_rules_dict = {"equal": ["value", "suit", "color",
                                      "is_royal", "even"],
                            "greater": ["value"],
                            "less": ["value"]}
    ...
    if rand <= 4:
        ...
        return "equal(" + val + "(" + card + "), " + random.choice(
            possible_value_dict[val]) + ")"
    elif rand <= 9:
        ...
        return (comparison + "(" + val + "(" + card1 + "), " + val +
                "(" + card2 + "))")
    else:
        # return a conjunctive rule
        conjunction = random.choice(operators)

        return (conjunction + "(" + getMiniRandomRule(6) + "," +
                getMiniRandomRule(6) + ")")
